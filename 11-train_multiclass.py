...
class Config:
    train_npy_dir = r"E:\mzf-data\train_preprocessed"
    val_npy_dir   = r"E:\mzf-data\valtest_preprocessed"
    test_npy_dir  = r"E:\mzf-data\valtest_preprocessed"
...
class NpyDataset(Dataset):
    def __init__(self, npy_dir, txt_file, is_train=False, oversample_severe=False):
        self.npy_dir = npy_dir
        self.is_train = is_train
        self.oversample_severe = oversample_severe
        self.samples = []
        ...
        if self.is_train and oversample_severe:
            severe_indices = [i for i, (_, label) in enumerate(self.samples) if label == 3]
            if len(severe_indices) > 0:
                target_count = max(np.bincount(self.labels)) * 2
                repeat_times = max(1, target_count // len(severe_indices))
                new_samples = []
                for idx in severe_indices:
                    new_samples.extend([self.samples[idx]] * repeat_times)
                self.samples.extend(new_samples)
                print(f"重度过采样：重度样本从 {len(severe_indices)} 复制到 {len(severe_indices)*repeat_times}")

        self.labels = np.array([s[1] for s in self.samples])
        print(f"最终样本数: {len(self.samples)}，标签分布: {np.bincount(self.labels)}")
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            if label == 3 and np.random.rand() > 0.5:
...
class COPDPipeline:
    ...
    def _init_data(self):
        self.train_dataset = NpyDataset(self.config.train_npy_dir, self.config.train_txt,
                                        is_train=True, oversample_severe=True)
        self.val_dataset   = NpyDataset(self.config.val_npy_dir,   self.config.val_txt,
                                        is_train=False, oversample_severe=False)
        self.test_dataset  = NpyDataset(self.config.test_npy_dir,  self.config.test_txt,
                                        is_train=False, oversample_severe=False)
        ...
        assert np.array_equal(np.unique(all_labels), np.arange(self.num_classes)), \
            "标签必须为 0,1,2,... 连续整数"
    ...
    def _init_model(self):
        self.model = resnet18_3d(num_classes=self.num_classes).to(self.config.device).float()
        train_labels = self.train_dataset.labels
        class_counts = np.bincount(train_labels, minlength=self.num_classes)
        class_weights = 1.0 / (class_counts + 1e-6)
        class_weights[3] *= 10.0
    ...
    def calculate_metrics(self, preds, labels):
        ...
        label_list = list(range(self.num_classes))

        metrics = {
            "acc": accuracy_score(labels_np, pred_labels),
            "kappa": cohen_kappa_score(labels_np, pred_labels),
            "macro_f1": f1_score(labels_np, pred_labels, average="macro"),
            "weighted_f1": f1_score(labels_np, pred_labels, average="weighted"),
            "confusion_matrix": confusion_matrix(labels_np, pred_labels, labels=label_list)
        }
        try:
            metrics["macro_auc"] = roc_auc_score(labels_np, probs,
                                                 multi_class="ovo", average="macro")
            metrics["weighted_auc"] = roc_auc_score(labels_np, probs,
    ...
    def run(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            if current_metric > best_val_acc:
                ...
                save_path = r"C:\Users\86187\best_resnet18_multi.pth"
        ...
        print("\n加载最佳模型，在测试集上评估...")
        self.model.load_state_dict(torch.load(r"C:\Users\86187\best_resnet18_multi.pth"))
